...
$Functions = new DatabaseClasses;
    ...
    if (isset($_GET['do-signUp'])){
        $data = $_POST['data'];
        // print_r($data);
        $id = $Functions->PDO('tbl_applicant','id');
        $name = $data[0]['value'];
        $email = $data[1]['value'];
        $password = sha1($data[2]['value'] );

        $query = $Functions->PDO("INSERT INTO tbl_applicant(id,lname,fname,mname,address,contactno,image,description,resume,email,password,gender) VALUES('{$id}','{$name}','','','','','','','','{$email}','{$password}','')");;
        if($query->execute()){
            echo 1;
        }
        else{
